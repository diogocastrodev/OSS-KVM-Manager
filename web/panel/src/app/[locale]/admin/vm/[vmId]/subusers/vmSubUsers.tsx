...
type Role = "operator" | "viewer" | "owner";
...
export default function VMSubUsers({ vmID, translations: t }: props) {
  ...
  const {
    ...
    queryFn: async () =>
      (await apiFetch(`/api/v1/admin/vms/${vmID}/subusers`)).json() as Promise<
        GetAllUsersResponse[]
      >,
  });

  const createSubUserSchema = z.object({
    email: z.email({ message: t.add.invalidEmail }),
    role: z.enum(["operator", "viewer", "owner"], {
      message: t.roleError,
    }),
  });
  ...
  const deleteUser = useMutation({
    ...
    mutationFn: async (subUserId: string) => {
      return apiFetch(`/api/v1/admin/vms/${vmID}/subusers`, {
        method: "DELETE",
